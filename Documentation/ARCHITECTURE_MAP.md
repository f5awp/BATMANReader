...
- ✅ **R3-H2 person-prior scoring (θ now live)** — `PersonPrior.logOdds(accepted:declined:)` (pure, Laplace-smoothed, clamped ±2, tested + teeth) + `LegFeatures.personPrior` + `TradeScore.wPerson` term in `legLogit`. Wired into the live Intents feed: `MessagingStore.partnerAcceptanceLogOdds` → `TradePackage.partnerPrior` (set in `intentSolutions`, avg over participants) → a `rankIntentPackages` **tiebreaker** (after intent/people/bookends) so, all else equal, partners who historically accept rank first. (Was θ=0/off.)
- ✅ **R3-B2 qual-swap ↔ base merge (lifecycle + UI)** — pure `TradeMerge.findBase(for:in:)` + `MessagingStore.mergeRequests(base:bridge:)` (saves the merged record, archives both originals — reversible, no split-brain) + `mergeBase(for:)` (gates on the bridge leg being `.finalized` and an active clean base sharing the give-day). `ThreadView` qual-swap section shows the giver a **"Merge with base trade"** button. Pure parts fail-tested + teeth (`findBase`); lifecycle build-verified.
- ✅ **R3-#9 Reddit-style nested replies** — `BroadcastReply.parentReplyID` (rides the JSON `payload`, no schema deploy; round-trip verified) + `MessagingStore.addReply(parentReplyID:)`. PURE `ReplyThread.flatten` (pre-order tree, siblings oldest→newest, orphan+cycle safe — tested + teeth) and `ReplyThread.subtreeIDs` (per-comment collapse). `ChannelView.threadedReplies/threadedReplyRow` render the tree: one threadline rail per depth, per-comment **Reply** (inline composer → nests under that comment) + **Hide/Show** subtree collapse. Indent capped at depth 6.
- ✅ **R3-Intents marketplace engine (distinct from Trade Solutions)** — `TradeRouter.intentSolutions(excluding:generation:)` is the Intents feed's OWN engine — NOT `packages()`. It's a marketplace of intent-for-intent deals involving you: per peer it runs `twoWayExplore`, splits each side's coverable days into MARKED (`wanted`) vs PREF, and `assembleIntentDeal` (PURE, tested) builds the best balanced two-person deal that maximizes mutual marked intent — seeding from EITHER side's marked days (a peer's marked day seeds a deal even if you marked no give). `rankIntentPackages` (PURE, tested) sorts **most mutual intent first** (vs `rankPackages`' fewest-people-first). Heavy 3+/circular is gated to Lucky via `generation` (U-PERF). The WHOLE roster is an eligible counterparty — the INTENT side must be someone who actually marked (the `assembleIntentDeal` seed), but the other side may be an unprofiled peer joining via PREFERENCES (bookend-only default, A8); low-intent deals just score lower and fall off the **top-20 cap** (`intentResultCap`). 3+ loops (`nWayRoutes(allowPrefMiddles:)`) need only **≥1 marked leg** (the marked seed guarantees it); non-seed participants join on preferences; `fireCount` = the REAL marked-leg count so all-intent loops rank highest **without hard-coding** all-marked. Set-contiguity (`anchoredSet`) enforced for `.bookends` parties. Feed wired (`TradeByIntentsFeed.reload`), header "Intent Matches". Red proven (marketplace-seed guard; cap contract-tested).
...
- ✅ **R3-UNIFIED SCORING (packageLogProb engine)** — every trade (2/3/4-person, circular, min-cost) now scored by ONE model. `LegFeatures` = `wantToTake`+`wantToTrade`+`bookend` (+intentLevel 0/1/2). `TradeScore.legLogit`: WANTS dominate (1.5 each → dual 3.0), bookend +0.8, **intent-scaled split penalty** `2.5−1.1·intentLevel` (none 2.5/single 1.4/dual 0.3 — so only mutual intent overrides a split), qual −1.2, personPrior 0.2. `packageLogProb` = Σ log(legProb) **+ N-penalty `(N−2)·log(0.85)`** (smaller trades dominate: `allDual+book(N+1) < allDual+split(N)`). `TradeRouter.legFeatures`/`packageLogProb(for:)` build REAL per-leg grades from live data; `intentSolutions`+`packages` set `acceptanceScore` = real packageLogProb → **`finalize`** score-orders, drops below the floor (**normal 0.32 / Lucky 0.07** via `lucky` flag), top-N empty-feed fallback, 60 ceiling. **N-max toggle** `SettingsManager.normalMaxPeople` (2/3/4, default 3) drives the normal feed's search depth (the old 2-way-only U-PERF gate is REVERSED; n-way `maxRoutes`=500 backstop + cancellable). Ladder verified (DB .99/DS .96/SB .94/NB .77/SS .62/NS .11). Tested + teeth. *(Removed: qualityScore band, synthetic packageScore, capped. Qual-swap + ECB are exempt — show-all pickers.)*
- ✅ **R3-A1 best-first search + variable score-floor cap** — `bestFirstSeeds` (PURE, tested+teeth) orders n-way seeds by `seedScore` = urgency (dominant) + TradeScore.legProb of a representative leg (date recency + qual friction); wired into `nWayRoutes`' seed loop. Route cap is now `maxRoutes` = **60 baseline / 100 Lucky** (`generation == .fast ? 60 : 100`). **Variable score-floor cap** for ALL ranked package match types: `TradePackage.qualityScore` (deterministic fire*3+bookends+fewer-people+urgency + TINY `TradeScore.qualityBlendWeight=0.05` so TradeScore is promoted-but-mostly-silent) → `scoreFloored(packages, band)` keeps everything within `band` of the best (self-scaling, "keep going" if many good) → single shared `capped(ranked, generation)` gate used by `intentSolutions` + `packages`. Bands: baseline **3** (~1 🔥) / Lucky **9** (~3 🔥); `intentResultCap=60` is now a safety ceiling. Broadcast pickers (qual-swap button, ECB) stay **show-all** by design (user choice). Tested + teeth. *(Follow-on: cancellable Lucky Task, best-first at every DFS node, band tuning on device.)*
- ✅ **R3-A1 "I'm Feeling Lucky" (complete)** — `MasterFilterSheet` (engine / max-people / Connection) with **Generate** (one-time heavy search) + **Reset**; button label shows the active criteria. The deeper search is now wired: **best-first seeding AND best-first at every DFS node** (`bestFirstSeeds`/`givePromise`), **cancellable** (`Task.isCancelled` + `runSearch` supersede-not-race), and the floor/score gate (see UNIFIED SCORING). Build-verified UI; pure cores tested.
...
- 🔨 **§U Unified trade engine + Solutions UI** (spec in NEXT_CHANGES §U). Build order 1–10, test-first:
  ...
  - ✅ **step 9 (UI)** — ~~"Just 2" segment~~ **REMOVED** (redundant with the Trade-size → Pairs toggle). Its full-roster **"Look up a dispatcher"** dropdown moved into `FindCandidatesSection` (Trade Solutions). Trades is now three tabs: Intents · Trade Solutions · ECB.
  - ✅ **U-PERF Trades caching** — `TradeFeedCache` (per-tab snapshot keyed by an input signature = intents revision + max-people + What-If + selected days) keeps each feed loaded across tab switches; the auto-running Intents feed restores instantly and re-runs ONLY when the signature changes. Searches are now **cancellable** (Cancel button + `Task.isCancelled`/`Task.yield()` in the per-candidate engine loops, so the main thread never freezes). Full-roster lookup loaded once per session (cached).
...
- ✅ **Z2 welcome/changelog** — startup `WelcomeView` (hero/purpose/pillars/what's-new + deep links to `MechanismsView` engineer-tour and `VersionHistoryView`); content is SINGLE-SOURCED in `AppGuide` (`purpose`/`pillars`/`mechanisms`/`versionHistory`), reopenable from Settings → "Welcome & how it works". Show-once gate = pure `ChangeLog.shouldShow` (tested, `AppInfo.build` vs `lastSeenChangelogBuild`). The old `ChangeLogEntry`/`ChangeLog.current`/`ChangeLogView` were removed to eliminate a duplicate changelog that could drift. (T45)
