...
- ✅ **R3-C1 SAVE-gated recompute (phase 1)** — `DayIntentStore.intentsRevision` + `markIntentsSaved()`; a green **Save** button on Home (marking header) bumps it + publishes. The Intents feed and Trade Solutions now recompute on `intentsRevision` change **instead of** `MatchInputsSignature` (which re-ran the heavy search on every edit). So editing intents no longer re-runs the search constantly — you SAVE, then it re-runs once. Red proven → 2 fail-tests. *(Phase 2 — full staging buffer / discard-on-no-save / nav-guard — is the follow-on.)*
