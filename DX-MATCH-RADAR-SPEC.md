...
### 4d. Proposals carry alternates (counter-ready) — GLOBAL to all calendar-view trades
**Applies to EVERY trade proposed from the calendar view** — the per-day **Trade List** (§3, section A) *and* the passive **Matches lane** (§4c) — not just passive matches. Even though you pick a specific day to propose, the request carries the **full candidate set**: your selected day AND the other legal alternate days from the sorted list. On the recipient's card the **selected day is highlighted**, and the **alternates remain visible so they can counter with a different day**; likewise you (the proposer) see the alternates you offered. Their counter comes back as a package card in one merged thread (Trade-Inbox Stage 8). So a propose is always "here's my pick, and here's what else works" — never take-it-or-leave-it. Store the alternates on the request (optional `candidateDayIDs` field, JSON payload, back-compat) so the recipient's counter picker is seeded from them. **Both parties always see alternates, not just the selected day.**
