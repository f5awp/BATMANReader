...
enum TradeRouter {
    ...
    static func packages(forGiveShifts giveShifts: [Shift], excluding selfID: String,
        ...
        // `*Tiers` map each ranked day → its display band (from `rankLegs`) for the `|` dividers.
        struct PeerSwap {
    ...
    /// Everything the radar produces in a single pass over the roster.
    struct RadarResult: Sendable {
        var pickupDays: Set<String> = []     // direction A — off-day pickups → the calendar STAR
        var takerDays: Set<String> = []      // direction B — my working days a want-to-work taker exists for
        var matches: [RadarMatch] = []       // mutual (both marked)
        var dayIndex: [String: DayRadar] = [:]
    }

    /// The radar scan. Builds `MatchContext` once (its CPU derive is already off-main), then runs the whole
    /// per-peer sweep OFF the main actor (`Task.detached` over Sendable snapshots) so neither the recompute
    /// nor a day tap ever blocks the UI. One pass yields the stars, the takers, the mutual matches, AND the
    /// per-day row index. NO finalize/floor/cap/loops.
    static func radarScan(excluding selfID: String) async -> RadarResult {
        let ctx = await MatchContext.build(selfID: selfID)
        let mySeeking = DayIntentStore.shared.seekingDayIDs
        let myProfile = TradeProfileStore.shared.myProfile()
    ...
    // (The per-day Trade List is now served from `MatchStore`'s precomputed `dayIndex` — see `radarScan`'s
    // RadarResult.dayIndex + `MatchStore.rows(forDay:)` — so tapping a day is an O(1) lookup, not a scan.)
    ...
    ///   1. rank TIER (below), which keeps intents highest and groups the `|` dividers
    ///   2. SOONEST within a tier (an equally-good earlier day is never buried under a later one)
    ///
    /// The tier is CHOOSER-CENTRIC — it reflects whether a day suits ME (the person picking):
    ///   0  both sides marked an intent (mutual)          · 1  one side marked an intent
    ///   2  no intent but it SUITS me                       · 3  doesn't suit me (unpreferred / bottom)
    /// "Suits me" depends on the side. On the GIVE side (I'm the giver) it's a CLEAN GIVE-AWAY — giving the
    /// day extends MY break (a neighbour is off), computed on MY *current* schedule (so a day stays a good
    /// give until I actually take an adjacent day back). On the GET side (I'm the receiver) it's whether I'd
    /// actually PICK IT UP under my soft prefs — blacklist / openness / weekend / qual / shift. A get-day I
    ...
    nonisolated private static func rankLegs(_ legs: [TwoWayLeg], giverID: String, receiverID: String,
        ...
        // HARD FILTER: a day the RECEIVER would NEVER accept is REMOVED entirely (not just demoted) — the
        // genuine "never" gates only: a Must-Be-Off day, a blacklisted weekday / desk / shift-type / region,
        // or a blacklisted QUAL (desk's required qual flagged 0). Everything softer — openness, bookend
        // contiguity, AND published-availability pills — is a RANKING signal (via `fits`/wouldPickUp → tier 3),
        // never a hard removal. (Treating pills as an exhaustive allowlist here zeroed the single-peer Find
        // Trades for anyone who'd posted a few pills — B6-FINDTRADES. Pills still push non-pilled days down.)
        func receiverHardBlocks(_ l: TwoWayLeg) -> Bool {
            ...
            return !receiverProfile.passesBlacklist(weekday: cal.component(.weekday, from: l.date), desk: l.desk,
        ...
        let sorted = scored.sorted { a, b in
            ...
            return a.leg.dayID < b.leg.dayID                       // within a tier: SOONEST first
        }
